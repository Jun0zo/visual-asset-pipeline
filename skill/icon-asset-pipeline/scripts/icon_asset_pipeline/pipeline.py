...
from .cleanup import clean_icon_crop
from .detection import detect_icon_candidates
from .models import BoundingBox, IconCandidate, IconRecord, InputAnalysis, PipelineConfig, ValidationIssue
from .naming import generate_icon_names, tags_for_icon
from .normalization import normalize_icon
from .packaging import write_package
from .segmentation import foreground_mask
from .validation import dominant_colors, find_duplicates, style_validation, validate_icon
...
def _dedupe(records: list[IconRecord], threshold: int) -> tuple[list[IconRecord], list[dict[str, str]]]:
...
def _record_from_candidate(
    ...
    issues = validate_icon(name, largest)
    return IconRecord(
        candidate=candidate,
        name=name,
        ...
        tags=tags_for_icon(name, prompt, colors),
        issues=issues,
...
def run_extract_pipeline(input_path: Path, output_dir: Path, config: PipelineConfig) -> dict[str, object]:
    ...
    candidates, mask = detect_icon_candidates(image, config, mask=initial_mask)
    ...
    for candidate in candidates:
        ...
        clean_crop, cleanup_notes = clean_icon_crop(crop, repair=config.repair)
        normalized = normalize_icon(clean_crop, config.normalized_sizes(), config.padding_ratio)
    ...
    names = generate_icon_names(candidates, representative_images, prompt=config.prompt, names_file=config.names_file)
    records = [
...
def run_directory_pipeline(input_dir: Path, output_dir: Path, config: PipelineConfig, normalize: bool = True) -> dict[str, object]:
    """Normalize and package an existing directory of individual icon files."""
    ...
    for index, path in enumerate(image_paths):
        ...
        clean_crop, cleanup_notes = clean_icon_crop(image, repair=config.repair)
        normalized = normalize_icon(clean_crop, config.normalized_sizes(), config.padding_ratio) if normalize else {
            size: clean_crop.resize((size, size)) for size in config.normalized_sizes()
        }
        candidate = IconCandidate(index=index, box=BoundingBox(0, 0, image.size[0], image.size[1]), confidence=1.0)
