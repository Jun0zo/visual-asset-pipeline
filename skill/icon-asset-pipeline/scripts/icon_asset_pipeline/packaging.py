...
from .models import IconRecord, InputAnalysis, PipelineConfig
...
def _write_sprite(records: list[IconRecord], size: int, output_dir: Path) -> dict[str, Any] | None:
    if not records:
        return None
...
def _write_contact_sheet(records: list[IconRecord], output_path: Path, size: int = 160) -> None:
    if not records:
        return
...
def write_package(
    ...
    metadata = {
        "title": f"{category.title()} Icon Asset Package",
        "description": config.prompt or "Icon assets extracted and normalized from source imagery.",
        "category": category,
        "source": source_path,
    ...
    for record in records:
        icon_meta = {
            ...
            "description": config.prompt or f"Normalized icon asset: {record.name}",
            "category": category,
            "tags": record.tags,
            ...
            "files": files_by_icon[record.name],
        }
        metadata["icons"].append(icon_meta)
        validation_report["icons"][record.name] = [issue.to_dict() for issue in record.issues]
    ...
    if config.zip_output:
        zip_path = output_dir / "icon_asset_package.zip"
