...
from icon_asset_pipeline.pipeline import run_extract_pipeline
...
def test_extract_pipeline_smoke(tmp_path: Path) -> None:
    ...
    result = run_extract_pipeline(
        source,
        output,
        PipelineConfig(sizes=(128, 256), expected_count=6, prompt="Create 6 forest exploration icons.", zip_output=True),
    )
    ...
    assert all(not icon["filename_base"].startswith("icon") for icon in metadata["icons"])
