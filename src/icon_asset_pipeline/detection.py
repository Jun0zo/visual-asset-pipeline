...
from .models import BoundingBox, IconCandidate, PipelineConfig
...
def _assign_grid_positions(candidates: list[IconCandidate]) -> None:
    if not candidates:
        return
...
def detect_icon_candidates(image, config: PipelineConfig, mask: np.ndarray | None = None) -> tuple[list[IconCandidate], np.ndarray]:
    """Locate icon candidates from visual foreground components, not equal grid cells."""
    ...
    if not raw_boxes:
        candidates = [IconCandidate(index=0, box=BoundingBox(0, 0, width, height), confidence=0.1, notes=["fallback_full_image"])]
    ...
    candidates = [
        IconCandidate(index=i, box=box, confidence=_candidate_confidence(box, (width, height)))
