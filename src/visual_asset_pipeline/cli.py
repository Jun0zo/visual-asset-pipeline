...
from visual_asset_pipeline.generation import write_generation_brief
from visual_asset_pipeline.models import ASSET_PROFILES, PipelineConfig
from visual_asset_pipeline.pipeline import run_directory_pipeline, run_extract_pipeline
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate briefs, extract, normalize, and package production visual assets.")
    subparsers = parser.add_subparsers(dest="command", required=True)

    brief = subparsers.add_parser("brief", help="Create a prompt brief for visual asset sheet generation.")
    ...
    brief.add_argument("--profile", choices=ASSET_PROFILES, default="auto")

    def add_common(subparser: argparse.ArgumentParser) -> None:
        subparser.add_argument("--output", required=True, type=Path)
        subparser.add_argument("--profile", choices=ASSET_PROFILES, default="auto")
    ...
    extract = subparsers.add_parser("extract", help="Extract visual assets from one source sheet and package them.")
    ...
    normalize = subparsers.add_parser("normalize", help="Normalize and package a directory of individual visual assets.")
    normalize.add_argument("--input", required=True, type=Path)
    add_common(normalize)

    package = subparsers.add_parser("package", help="Package a directory of visual assets with optional normalization.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "brief":
        path = write_generation_brief(args.prompt, args.output, count=args.count, profile=args.profile)
