...
from .models import AssetRecord, ValidationIssue
...
def validate_asset(name: str, image: Image.Image) -> list[ValidationIssue]:
    arr = rgba_array(image)
    alpha = arr[:, :, 3]
...
def find_duplicates(records: list[AssetRecord], threshold: int = 7) -> dict[str, list[str]]:
    hashes = {
...
def style_validation(records: list[AssetRecord]) -> dict[str, object]:
    if not records:
        return {"status": "empty", "warnings": ["no_assets"]}
